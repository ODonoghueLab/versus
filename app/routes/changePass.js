...
module.exports = (app) => {
  ...
  app.post('/account', routeAuth.isAuth, (req, res) => {
    ...
    if (!req.body.currentPassword) { res.render('error'); }
    else if (!req.body.newPassword) { res.render('error'); }
    else if (!req.body.newPasswordConfirm) { res.render('error'); }
    else if (req.body.newPassword !== req.body.newPasswordConfirm) { res.render('error'); return null; }
    else {
